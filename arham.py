...
def find_intersections(data, levels, is_support=True):
    """
    Find and analyze lines between points in the support/resistance levels.

    Parameters:
        data (DataFrame): Stock data with high and low prices.
        levels (Series): Support or resistance levels.
        is_support (bool): If True, process support levels, otherwise resistance.

    Returns:
        List of tuples: Contains points and intersection status.
    """
    points = levels.dropna()
    points_indices = points.index
    intersections = []

    for i in range(len(points_indices) - 2):
        # Select points
        x1, x3 = points_indices[i], points_indices[i + 2]
        y1, y3 = points[x1], points[x3]
        ...
        for x in points_indices[i : i + 2]:
            y_line = m * x.toordinal() + c
            if is_support:
                if y_line >= data.loc[x, "High"]:
                    intersections.append((x1, x3, "intersection"))
                    break
            else:
                if y_line <= data.loc[x, "Low"]:
                    intersections.append((x1, x3, "intersection"))
                    break
        else:
            intersections.append((x1, x3, "no intersection"))

    return intersections


# Support and Resistance Levels
support_levels_clean = support_levels.dropna()
resistance_levels_clean = resistance_levels.dropna()

# Analyze Support Lines
support_analysis = find_intersections(data, support_levels_clean, is_support=True)
resistance_analysis = find_intersections(
    data, resistance_levels_clean, is_support=False
)

# Print Analysis Results
print("Support Analysis:")
for sa in support_analysis:
    print(f"Points {sa[0]} to {sa[1]}: {sa[2]}")

print("\nResistance Analysis:")
for ra in resistance_analysis:
    print(f"Points {ra[0]} to {ra[1]}: {ra[2]}")
